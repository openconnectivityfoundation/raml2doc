...
# Adding this to add !include support in yaml parser
...
def parse_resource(c, property_name, parent_object):
    ...
    resource.queryParameters = new_context.get_property_with_schema("queryParameters", RamlMethod.queryParameters)
...
def parse_traits(c, property_name):
    ...
    for trait_raw_value in property_value:
        #print "parse_traits:::",property_value
        
        
        
        for pvtrait in property_value:
            #print pvtrait
            trait = RamlTrait()
            
            for trait_name, obj in pvtrait.items():
                #print trait_name, obj
                for objname, obj in obj.items():
                    #print objname, obj
                    if objname == "queryParameters":
                        trait.queryParameters = obj
                    if objname == "queryParameters":
                        trait.queryParameters = obj
...
def parse_body(c):
    ...
    # dirty fix
    c = ParseContext(c.get("application/json"), c.relative_path)
