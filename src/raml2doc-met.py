...
class CreateDoc(object):
    def __init__(self, name, docxName=None, resourceName=None):
        ...
        self.schema_ignorelist = ['required','$schema','type', 'definitions','description',
            'properties',":",":{","minItems","maxItems", "maxLength", "attribute","format","allOf", "$ref", "enum",
            "title", "oneOf", "anyOf","additionalProperties", "items", "minimum", "maximum", "pattern", "readOnly"]
    ...
    def listResource(self, level, resource, obj):
        ...
        if obj.methods != None:
            ...
            # Always set Notfy as all resources are observable in non-Core cases
            if self.annex_switch == False:
                row_cells[5].text = "get"
            ...
            if " " not in tokens[1]:
                if len(tokens) > 5:
                    if tokens[1] not in self.schema_ignorelist:
                        print "TP#1. Tokens: ", tokens
                        row_cells = self.tableAttribute.add_row().cells
                        row_cells[0].text = tokens[1]
                        # By default tag as Read Write and overwrite if needed
                      ...
                      if " " not in tokens[1]:
                        if "oic." not in tokens[1] :
                            ...
                            if "id" == tokens[1] and ignore == True:
                                ...
                                # By default tag as Read Write and overwrite if needed
                                row_cells[3].text = "Read Write"
                                print "TP#2. Tokens: ", tokens
                                print "TP#3. Tokens2: ", tokens2
                                print "TP#4. Tokens3: ", tokens3
                                print "TP#5. Tokens4: ", tokens4
                                ...
                                if len(tokens3) > 1:
                                    if "readOnly" in [tokens3[1]]:
                                        row_cells[3].text = "Read Only"
                                if len(tokens4) > 1:
                                    if "readOnly" in [tokens4[1]]:
                                        row_cells[3].text = "Read Only"
                                if len(tokens3) > 3:
                                    # description on line 3...
                                    if tokens3[1] == "description":
                                        description_text = tokens3[3]
                                        if len(description_text) > 0:
                                          if description_text[0].isupper() == False:
                                            description_text= description_text.title()
                                        # overwrite text
    ...
    def generateSections(self, parsetree , sectionName=None):
        ...
        if self.annex_switch == False:
            ...
            par = self.document.add_heading('Fixed URI', level=3)
        if self.annex_switch == True:
            par.style = 'ANNEX-heading2'
